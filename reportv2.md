...
昨天深夜，我和森森在微信上对着小草神的涩图指指点点——这张光影不行，那张表情不够涩。正当剑拔弩张之际，森森突然甩过来一张**奶龙教培专用 GIF**，然后问了一句改变历史的话：
...
**Nginx 413 暗藏杀机。** 签名墙前面有个 nginx，默认 `client_max_body_size` 大约 1MB。第一次没缩放，APNG 膨胀到 1.4MB，直接被拦在门外。把 GIF 缩到 200px 宽才控制在 300KB 以内。令人高兴的是，森森表示：
> “要是取消限制，是不是就可以上传篇了？”

内鬼消息孩子们！大小上传限制将被取消！届时我将给大家带来脚本v2.2的更新！
版本前瞻：原图无损发布功能、超长GIF支持（那该怎样用格式工厂或AdobeMediaEncoder将篇转成GIF呢？完全想不到呢~🤔）
...
- **不跳过用户确认。** GIF 选择后显示首帧预览，**必须填昵称（也可留空） + 点「贴到墙上」才会提交**，不会偷偷发出去
...
- GIF 超过 200 帧会提示精简（再多也没有，谁家 GIF 200 帧，篇另说）
- 最终 APNG 超过 900KB 会拒绝（nginx 不吃这套，听说后续有更新）
...
以上所有操作均持续获得站点所有者口头授权（*"你能传上去算你厉害"*——森森，2026年某月某日深夜，微信语音）。
